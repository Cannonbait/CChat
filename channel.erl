...
initial_state(Id) ->
	...
	Users = ordsets:new(),
    #channel_st{users= ordsets:add_element(Id, Users)}.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% 		INTERNAL

%Put user in channel
loop(State, {join, {Id}}) ->
	{{join, ok}, State#channel_st{users = ordsets:add_element(Id, State#channel_st.users)}};

%Recieve message from user
loop(State, {message, {Nick, Id, Channel, Msg}}) ->
	%Spawn new process to send messages
	spawn(channel, sendMessage, [ordsets:to_list(State#channel_st.users), {Channel, Nick, Msg, Id}]),
...
loop(State, {leave, {Id}}) ->
	{{leave, ok}, State#channel_st{users = ordsets:del_element(Id, State#channel_st.users)}}.
